...
from recsys_gen.data.sequences import build_user_sequences, sample_negative_items, write_user_sequences
...
def test_write_user_sequences_streams_parquet_output(tmp_path: Path) -> None:
    frame = pl.DataFrame(
        {
            "user_id": [1, 1, 1, 2, 2, 2],
    ...
    write_user_sequences(frame, max_length=3, min_length=2, output_path=output_path, batch_rows=1)
    sequences = pl.read_parquet(output_path)
    assert sequences.height == 2
    assert sequences["history"].to_list() == [[10, 11], [20, 21]]
